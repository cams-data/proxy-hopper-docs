...
│  hot-reload ← ProxyRepository change events                  │
...
**`ProxyRepository`** — CRUD store for all runtime entities: targets, IP pools, and providers. Persists objects as JSON in the backend KV and publishes change events (`add`/`update`/`remove`) over a pub/sub channel. Owns the cascade chain: a provider IP change propagates through pools to recompute target `resolved_ips` snapshots and emits `target:update` events. Used by `ProxyServer` to hot-reload without restart and by the GraphQL API for all mutations.
...
**Backend abstraction** — `Backend` is a clean storage interface. Swapping backends requires no changes above the domain store layer. Domain stores (`IPPoolStore`, `ProxyRepository`) own key naming and calling conventions; `IPPool` and `ProxyServer` only see the domain stores.
...
│   └── src/proxy_hopper/
│       ├── server.py           # ProxyServer — TCP server, auth check, dispatch, hot-reload
...
│       ├── repository.py       # ProxyRepository — CRUD + pub/sub + cascade for targets/pools/providers
│       ├── backend/
│       │   ├── base.py         # Backend ABC — storage primitives
│       │   └── memory.py       # MemoryBackend
│       ├── config/
│       │   ├── models.py       # Pydantic models — TargetConfig, IpPool, IpRequest, ProxyProvider, …
│       │   ├── loader.py       # load_config() — YAML → ProxyHopperConfig
│       │   └── normalization.py# camelCase → snake_case normalisation helpers
│       ├── graphql/
│       │   ├── queries.py      # Query type — targets, pools, providers, status
│       │   ├── mutations.py    # Mutation type — CRUD + addIpToProvider / removeIpFromProvider
│       │   ├── types.py        # Strawberry output types
│       │   └── inputs.py       # Strawberry input types + converters
│       ├── auth/
│       │   ├── __init__.py     # authenticate_token, can_access_target, JWT helpers
│       │   └── admin.py        # FastAPI admin app (REST + GraphQL mounted at /graphql)
│       ├── identity/
